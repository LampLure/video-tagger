...
impl VideoTaggerApp {
    ...
    pub(super) fn render_sidebar(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical_centered(|ui| {
            ui.label(RichText::new("控制面板").strong().size(16.0));
        });
        ui.add_space(12.0);
    ...
    fn render_thumbnail_card(&mut self, ui: &mut egui::Ui, video_idx: usize, thumb_size: Vec2, card_size: Vec2) {
        ...
        let inner = card_frame(ui.style(), false, processed).show(ui, |ui| {
    ...
    fn paint_thumbnail(&mut self, ui: &mut egui::Ui, rect: egui::Rect, video_idx: usize, loading_text: &str) {
        if let Some(texture) = self.overview_thumbnails.get(&video_idx) {
            let img = egui::Image::new(texture).fit_to_exact_size(rect.size()).rounding(Rounding::same(4.0));
            ui.put(rect, img);
                    ...
                    for col in 0..cols {
                        ...
                        if idx < tag_names.len() {
                            ...
                            let btn = egui::Button::new(RichText::new("+ 新增").size(13.0).color(Color32::from_gray(150)))
                                ...
                                .rounding(Rounding::same(6.0))
                                .min_size(Vec2::new(btn_w, 32.0));
                            if ui.add(btn).clicked() { self.editing_new_tag = true; self.new_tag_text.clear(); }
    ...
    pub(super) fn render_ffmpeg_dialog(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("FFmpeg 环境配置")
            ...
            .frame(egui::Frame::window(&ctx.style()).fill(Color32::from_rgb(32, 34, 38)).rounding(8.0))
            .show(ctx, |ui| {
                ui.add_space(8.0);
                if let Some(ref path) = self.ffmpeg_path {
                    ui.horizontal(|ui| {
